...
export function tuneStyle(style) {
  const layers = style.layers.map((layer) => {
    ...
    if (layer.id === "background") {
      next.paint["background-color"] = "#0D1420";
    }
    if (layer.type === "fill" && /water/.test(layer.id)) {
      next.paint["fill-color"] = "#111B29";
    }
    ...
    if (layer.type === "line" && /road|highway|transportation/.test(layer.id)) {
      next.paint["line-color"] = next.paint["line-color"] ?? "#2A3444";
      next.paint["line-opacity"] = 0.8;
    }

    // Label text: warm off-white, legible against the dark basemap.
    if (layer.type === "symbol" && layer.layout?.["text-field"]) {
      next.paint["text-color"] = /road|highway/.test(layer.id)
        ? "#9AA5B5"
        : "#E7E9EE";
      next.paint["text-halo-color"] = "#0D1420";
