...
// --- Auto-updating recent data (DAT damage assessments, IEM warnings) ---
// Unlike the historical tornado/hurricane archive, these are single
// rolling-window files (not decade-split) refreshed daily by the same
// pipeline - see scripts/fetch-damage-assessment.js and
// scripts/fetch-warnings.js.
const recentCache = {};

async function loadOnce(key, path) {
  if (!recentCache[key]) {
    recentCache[key] = fetchJSON(path).catch((err) => {
      delete recentCache[key]; // allow retry on next call rather than caching a failure forever
      throw err;
    });
  }
...
export function loadDamagePoints() {
  return loadOnce("damage-points", "/data/damage/points.geojson");
}
export function loadDamageLines() {
  return loadOnce("damage-lines", "/data/damage/lines.geojson");
}
export function loadDamagePolygons() {
  return loadOnce("damage-polygons", "/data/damage/polygons.geojson");
}
export function loadDamageSummary() {
  return loadOnce("damage-summary", "/data/damage/summary.json");
}
export function loadRecentWarnings() {
  return loadOnce("warnings-recent", "/data/warnings/recent.geojson");
}
